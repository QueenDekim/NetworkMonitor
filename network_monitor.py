...
if __name__ == "__main__":
    while True:
        ...
        if choice == "1":
            ...
            start_api()  # Start the API
            network = get_user_input("Enter the network to scan " + Fore.CYAN + f"(e.g., {SCAN_CONFIG['DEFAULT_NETWORK']}): " + Fore.WHITE, f"{SCAN_CONFIG['DEFAULT_NETWORK']}")
            ports = get_user_input("Enter the ports to scan " + Fore.CYAN + f"(e.g., {SCAN_CONFIG['DEFAULT_PORTS']}): " + Fore.WHITE, f"{SCAN_CONFIG['DEFAULT_PORTS']}")
            interval = float(get_user_input("Scan interval (minutes)  " + Fore.CYAN + f"(e.g., {SCAN_CONFIG['DEFAULT_INTERVAL']}): " + Fore.WHITE, f"{SCAN_CONFIG['DEFAULT_INTERVAL']}"))
